...
void Pirate::Battle(Pirate enemyPirate, Ship enemyShip) {
  if (m_curShip.m_curToughness <= 0) { // if current ship toughness is less than or equal to 0
	...
	cout << "You Win!" << endl;
	CalcTreasure(enemyShip); // calculates treasure of emeny ship
      }
...
void Pirate::RepairShip() {                                                                                            
  ...
  m_pirateDays = m_pirateDays + damageShip; // adds days to fix damage of ship to pirate days
}
