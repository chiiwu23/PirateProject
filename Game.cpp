  ...
  if (inputStream.is_open()){
      ...
      if (x == 6)
	{
	  ...
	  if (arryCounter != MAX_SHIPS)
	    {
	      string newElement = "";
	      for (int x = element.size() -1; x> -1; x--)
		{
...
void Game::LoadPirates(){ // will load the pirates in to the array in order to use them
  ifstream inputStream; 
  ...
  if (inputStream.is_open()) {
    ...
    int x = 0; //Counter for splicing
    int arryCounter = 0;
    string elementPirates; // used to store elements of the pirate
    while (getline(inputStream, elementPirates, ',')) { // used to get lines of the .txt file
      if (x == 0) {
	...
	if (arryCounter != MAX_PIRATES - 1) 
	  {
	    int y = 0;
	    string descBuilder = "";
	    char charElement = elementPirates[y]; // character of the elements
	    while (charElement != '\n')
	      {
		descBuilder = descBuilder + charElement; 
	    ...
	    pirateDesc = descBuilder; // description of pirate is descBuilder now
	  }
	else
	  {
	    pirateDesc = elementPirates; // else it is element of pirate
	  }
...
void Game::StartGame(){
  GameTitle(); // calls game title when game starts
  LoadShips();  // load ships 
  LoadPirates(); // loads pirates
  srand(time(NULL)); 
  int piratesNum = rand() % MAX_PIRATES; //randomized pirates
  int shipsNum = rand() % MAX_SHIPS; //randomized ships
  
  m_myPirate = m_allPirates[piratesNum]; //randomizes pirates 
  Ship ship = m_ships[shipsNum]; // randomizes ships 
  m_myPirate.SetCurShip(ship); // sets pirate with ship 
  ...
  int choice = MainMenu(); // sets choice as MainMenu() function call
  while (choice != 4) { //
    if (choice == 1) {
      SearchTreasure(); // calls the function of SearchTreasure
    }
    else if (choice == 2) {
      m_myPirate.RepairShip(); // calls repair of pirate ship
    }
    else if (choice == 3) {
      m_myPirate.DisplayScore(); // calls display of pirate score
    }
    cout << endl;
    choice = MainMenu(); // created to be called once pirated has finished one of the othernitems on menu
  }
  cout << "**************************" << endl;
...
int Game::MainMenu() {
  ...
  do {
    ...
    cin >> userChoice;
    cout << endl;
  } while (userChoice < 1 && userChoice > 4); // while userchoice is greater than one and less than 4.
  return userChoice;
}
...
void Game::SearchTreasure(){
  srand(time(NULL));
	...
	enemyPirate = m_allPirates[rand() % MAX_PIRATES]; // will randomize the enemy pirates 
	enemyShip = m_ships[rand() % MAX_SHIPS]; // will randomize the enemy ship
	enemyPirate.SetCurShip(enemyShip); // sets enemy pirate with current ship of enemy ship
	cout << "Off in the distance, you see " << enemyPirate.GetName() << " on a " << enemyShip.m_type << "!" << endl;
	int choiceTarget = 0;
	while (choiceTarget != 1 && choiceTarget != 2) { // will only let user choose between 1 and 2
		cout << "What would you like to do?" << endl;
